  ...
  try {
    await fc.assert(fc.asyncProperty(fc.string({ maxLength: 4096 }), fc.string({ maxLength: 4096 }), async (first, replacement) => {
      ...
      for (const content of [first, replacement]) {
        const result = await writeTool.execute(JSON.stringify({ file_path: filePath, content }));
        expect(result).toEqual({ ok: true, value: { status: "success", content: "File written successfully." } });
